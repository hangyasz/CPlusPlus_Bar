...
//Kotek parameter nelkul konstruktor
Koktle::Koktle():alapanyag_db(0), alapanyagok(nullptr), menyiseg(nullptr){
    //az alapanyagok es a menyiseg tomboket nullpointerre allitjuk és az alapanyag_db-t 0-ra
}
...
void Koktle::removeAlapanyag(Ital *ital) {
    ...
    while (index!=0) {
            ...
            for (size_t i=0; i<index; i++){
                uj_alapanyagok[i] = alapanyagok[i];
                uj_mennyiseg[i] = menyiseg[i];
            }
...
//kiirja a koktelokat
void Koktlok::kiir(std::ostream& os) const {
    //vegigmegy a koktelokon es mindegyiket kiirja
    for (size_t i=0; i<this->koktel_db; i++){
        this->koktelok[i]->kiir(os);
    }
}
...
void Koktlok::removeKoktel(std::ostream &os, std::istream &is) {
    ...
    --index;
    if(koktel_db==0) {//ha nincsenek koktelok akkor kiirjuk hogy ures es visszaterunk
        os<<"urse nem tartalz koktelokat"<<std::endl;
        return;
    }
...
//Koktelok Destruktor
Koktlok::~Koktlok() {
    //vegigmegy a koktelokon es torli oket
    for (size_t i=0; i<this->koktel_db; i++){
        delete koktelok[i];
    }
